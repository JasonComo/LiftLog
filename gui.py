...
class GUI:
    ...
    def createCreateWorkoutFrame(self):
        ...
        def storeWorkout():
            ...
            for name, sets, reps in zip( exercise_names_values, exercise_sets_values, exercise_reps_values):
                ...
                if len(name) > 12:
                    messagebox.showerror("Error", "Exercise name cannot be more than 12 characters.")
                    return
    ...
    def createViewFrame(self):
        ...
        for i in range(num_workouts):
            rely = 0.35 + (i * 0.25) / num_workouts  
            view_button = Button(self.vw_frame, background="white", image=blank_btn, text=self.account.workouts[i].name, activebackground="white", activeforeground="white", compound="center", bg="white", fg="white", font=arial_2, borderwidth=0, command=lambda: viewWorkout(i))
    ...
    def createEditFrame(self):
        ...
        for i in range(num_workouts):
            rely = 0.35 + (i * 0.25) / num_workouts  # Calculate the rely position based on the number of workouts
